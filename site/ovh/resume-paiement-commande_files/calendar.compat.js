...
// Mootools 1.2.2 compatibility by Dorchies Philippe
// Mootools 1.4.1 compatibility by Leroux Ludovic
...
var Calendar = new Class({
	...
	initialize: function(obj, options) {
		...
		this.calendar = new Element('div', {
			'styles': { left: '-1000px', opacity: 0, position: 'absolute', top: '-1000px', zIndex: 100000 }
		}).addClass(this.classes.calendar).inject(document.body);

		// iex 6 needs a transparent iframe underneath the calendar in order to not allow select elements to render through
		if (window.ie6) {
			this.iframe = new Element('iframe', {
				'styles': { left: '-1000px', position: 'absolute', top: '-1000px', zIndex: 100000 }
        ...
        if( typeOf( obj ) == 'object' ){
            ...
            for( var i = 0; i < obj.length; i++ ){
                ...
                // fix for bad element (naughty, naughty element!)
                if (!this.element(obj[ i ].element, obj[ i ].format, cal)) { continue; }
	...
	toggle: function(cal) {
        ...
        this.calendars[ 0 ].el.removeEvent( 'blur', this.fn );
        document.removeEvent( fixEv, this._KeyPressed );
		...
		if (cal.visible) { // simply hide curr cal
            ...
            this._KeyPressed = function( e ){
                if( e.key == "esc" )    this.fn( e );
            }.bind( this );

			this.fn = function(e) {
				var e = new DOMEvent(e);
            ...
            if( coord.right != 0 ){
                ...
                var coordInput = cal.el.getCoordinates();
    			var x = coordInput.right + this.options.tweak.x;
	    		var y = coordInput.top + this.options.tweak.y;
            }
            ...
            this.calendars[ 0 ].el.addEvent( 'blur', this.fn );
            document.addEvent( fixEv, this._KeyPressed );
